...
contactRoute.get('/', async (req, res) => {
    try {
        ...
        //update view =1 in tbl_contact
        const [not_viewed_users] = await db.query("SELECT ContactID FROM tbl_contact WHERE view = 0");

        if (not_viewed_users.length > 0) {
             const userIds = not_viewed_users.map(user => user.ContactID);
    
             await db.query("UPDATE tbl_contact SET view = 1 WHERE ContactID IN (?)", [userIds]);
             console.log('Updated viewed users');
        }


        // await db.query("UPDATE tbl_contact SET view = 1 WHERE ContactID =?", [contacts[0].ContactID]);
