...
const sendNotifications = async () => {
  try {
      const [latestOrder] = await pool.query(
          "SELECT * FROM tbl_order WHERE OrderDate >= NOW() - INTERVAL 1 DAY AND view = 0  ORDER BY OrderDate DESC LIMIT 5"
      );
      const [latestUser] = await pool.query(
          "SELECT * FROM tbl_user WHERE created_at >= NOW() - INTERVAL 1 DAY AND view = 0  ORDER BY created_at DESC LIMIT 5"
      );
      const [latestContact] = await pool.query(
        "SELECT * FROM tbl_contact WHERE CreatedAt >= NOW() - INTERVAL 1 HOUR AND view = 0  ORDER BY CreatedAt DESC LIMIT 5"
    );
      const data = {
