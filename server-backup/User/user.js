...
userRoute.get('/', async (req, res) => {
  try {
    ...
    //update view to 1 in tbl_user
    const [users_view] = await db.query('SELECT id FROM tbl_user WHERE view = 0');

    if (users_view.length > 0) {
        const userIds = users_view.map(user => user.id);
    
        // Step 2: Update the selected users in bulk
        await db.query('UPDATE tbl_user SET view = 1 WHERE id IN (?)', [userIds]);
    }
